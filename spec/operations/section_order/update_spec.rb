...
RSpec.describe SectionOrder::Update do
  ...
  let!(:section_1) { create(:section, part:) }
  let!(:section_2) { create(:section, part:) }
  let!(:section_3) { create(:section, part:) }
  ...
  scenario "Having already set positions" do
    section_1.update_column(:position, 1)
    section_2.update_column(:position, 2)
    section_3.update_column(:position, 4)
    array = ["section_#{section_2.id}", "section_#{section_3.id}", "section_#{section_1.id}"]
    result = described_class.({ part_id: part.id, ordered_sections: array })
    expect(result.success?).to eq true
  end
